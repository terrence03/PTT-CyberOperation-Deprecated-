...
data_path = r"D:\\研究\\選舉研究\\資料\\活躍百分位數0.9_留言下限10則_文章不限單一候選人.xlsx"
...
polarity = polarity.groupby(["Region", "com_User"], as_index=False)[["KMT", "DPP", "OTHER"]
].sum()
...
def prefer_party(DateFrame, Criterion, multiple=1):
    ...
    for i in tqdm(range(len(polarity_1["com_User"]))):
        if (
            polarity_1.iloc[i, 3] >= polarity_1.iloc[i, 4] * multiple
            and polarity_1.iloc[i, 3] > 0
        ):
